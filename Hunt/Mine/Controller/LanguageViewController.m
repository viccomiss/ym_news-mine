...
- (void)setData{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        m.name = titles[i];
        if ([[[self language][i] substringToIndex:2] isEqualToString:[[[LocalizedHelper standardHelper] currentLanguage] substringToIndex:2]]) {
            m.selected = YES;
        }
...
//    [UserModel usr_setting_update:@{@"key" : @"locale", @"value": [self languages][indexPath.row]} Success:^(NSString *code) {
//
...
//
//    }];
    [SENotificationCenter postNotificationName:LANGUAGECHANGE object:[self language][indexPath.row]];
    switch (indexPath.row) {
        case 0:
            [UMAnalyticsHelper event:home_dyy_zw];
            break;
        case 1:
            [UMAnalyticsHelper event:home_dyy_yw];
            break;
    }
}
