...
//        _photoBrowserWindow.windowLevel = UIWindowLevelAlert;//2000的优先级,这样不会遮盖UIAlertView的提示弹框
        BaseViewController *tempVC = [[BaseViewController alloc] init];
...
- (void)updateFrames
{
    ...
    self.indexLabel.xl_centerY = MAINSCREEN_HEIGHT - 30;
...
//    [[UIApplication sharedApplication] setStatusBarHidden:YES withAnimation:UIStatusBarAnimationNone];
    self.photoBrowserWindow.rootViewController.navigationBar.hidden = YES;
    [self iniaialUI];
}
...
//    [[UIApplication sharedApplication] setStatusBarHidden:NO withAnimation:UIStatusBarAnimationNone];
    self.photoBrowserWindow.rootViewController.navigationBar.hidden = NO;
    [UIView animateWithDuration:XLPhotoBrowserHideImageAnimationDuration animations:^{
        self.alpha = 0.0;
        self.photoBrowserWindow.rootViewController.view.alpha = 0;
    } completion:^(BOOL finished) {
        [self removeFromSuperview];
        [self.photoBrowserWindow.rootViewController.view removeFromSuperview];
