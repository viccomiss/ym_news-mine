...
- (void)viewDidLoad {
    ...
    //主题切换监听
    [SENotificationCenter addObserver:self selector:@selector(changeTheme:) name:SKINCHANGESUCCESS object:nil];
}

//更新主题
- (void)changeTheme:(NSNotification *)notification{
    for (int i = 0; i < self.viewControllers.count; i++) {
        UIViewController *child = self.viewControllers[i];
        child.tabBarItem.selectedImage = [[ThemeManager imageForKey:_tabBarItemsAttributes[i][CYLTabBarItemSelectedImage]] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
    }
}
...
- (void)addOneChildViewController:(UIViewController *)viewController
    ...
    if (selectedImageInfo) {
        UIImage *selectedImage = [[ThemeManager imageForKey:selectedImageInfo] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
